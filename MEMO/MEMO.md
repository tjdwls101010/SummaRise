# ✅이 프로젝트는 Supabase에서 데이터를 불러와 메인페이지에서 리스트나 갤러리로 보여줘야 하는데, 아무 것도 안보임. @MEMO/Nothing.png


# ✅`설정` 버튼을 눌러도 이동이 안됨.

# ✅이 웹사이트 좌측 상단에 로고가 있는데 이 로고를 @MEMO/AppLogo.png 로 교체해주세요.

# ✅왼쪽 사이드바를 아래를 바탕으로 변경해줘.
<Reference>
...
# `Supabase MCP`를 사용해서 목업데이터를 생성하고, 웹사이트에서 이 데이터들을 실제로 가져와서 잘 디스플레이 하는지 `playwright MCP`를 사용해 확인해봐.

# ✅이 사이트에 로그인 기능은 필요없어요. 없애주세요.

# ✅`설정`페이지에 들어가면 `시스템 프롬프트`, `API 키` 등을 한 페이지에서 설정할 수 있게 해주세요. 버튼을 눌러서 다른 페이지로 넘어가도록 하는게 아니라요.
